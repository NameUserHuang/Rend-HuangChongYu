...
public class AppController {
	...
	public String addSave(AppInfo appInfo,HttpSession session,HttpServletRequest request,
		...
		if(!attach.isEmpty()){
			...
			int filesize = 500000;
			if(attach.getSize() > filesize){//原文件后缀
	...
	public String modifyAppVersionSave(AppVersion appVersion,HttpSession session,HttpServletRequest request,
		...
		if(!attach.isEmpty()){
			...
			String oldFileName = attach.getOriginalFilename();//原文件名
			String prefix = FilenameUtils.getExtension(oldFileName);//原文件后缀
			if(prefix.equalsIgnoreCase("apk")){//apkæä»¶å½åï¼apkåç§°+çæ¬å·+.apk
				 String apkName = null;
				 try {
					apkName = appInfoService.getAppInfo(appVersion.getAppId(),null).getAPKName();
				 } catch (Exception e1) {
	...
	//删除APP
	@RequestMapping(value="/deletapp.json")
	@ResponseBody
	...
	@RequestMapping(value="/sev")
	public Object Satus() {
		
		return "";
	}
	
	@RequestMapping(value="/sev",method=RequestMethod.POST)
	public Object updateSatus(@RequestParam Integer status,@RequestParam Integer id)throws Exception{
		
		return "redirect:/Satus";
	}
//	@RequestMapping(value="deletapp.json",method=RequestMethod.GET)
//	public Object delApp(@RequestParam String id) 
//	{
//		
//		
